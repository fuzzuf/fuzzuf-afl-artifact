...
// moreover, "whether BB coverage can be retrieved" should be independent from need_afl_cov.
// so we test both cases, need_afl_cov=false, need_afl_cov=true
static void InstrumentCovBB(bool forksrv, bool need_afl_cov) {
  ...
  for (int i=0; i<8; i++) {
      ...
      // moreover, normally the difference should be just one node(where "var = 1" or "var = 0" takes place)
  ...
  for (int bit_seq=0; bit_seq<lim; bit_seq++) {
      ...
      for (int i=0; i<8; i++) {
          ...
          if (is_ith_set) {
              ...
              // and that seed bit_seq should not have passed nodes in diffs[i].ignored
              for (int node_idx : diffs[i].ignored) {
                  BOOST_CHECK(cov.count(node_idx) == 0);
              }
          } else {
...
BOOST_AUTO_TEST_CASE(Test4CasesWithInstrumentCovBB) {
    for (int i=0; i<2; i++) {
        for (int j=0; j<2; j++) {
            ...
            if (j) std::cout << ", need_afl_cov=true" << std::endl;
            else   std::cout << ", need_afl_cov=false" << std::endl;
