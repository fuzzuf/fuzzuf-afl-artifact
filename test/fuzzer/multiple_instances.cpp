...
// fork server mode, non fork server mode双方テストするため、引数でどちらのモードを使うか指定
...
void LaunchInstance(bool forksrv, bool &result) {
    ...
    auto fuzzer = PythonFuzzer(
      { "../put_binaries/command_wrapper", "/bin/sleep", "4" }, // 4秒sleepする。ただしタイムアウトは3秒
...
// FIXME: 現状non fork server modeではこのテストは失敗するはずなので当座の処置としてfork server modeのみテストする
